...
И пришли к идее использования элементов нечеткой логики в обучении вариационных энкодеров (VAE).

Но начнем с небольшого лирического вступления.

Когда то давным давно, когда вариационные автоэнкодеры только только появились, это было как гром среди синего неба - наконец-то сетки смогут извлекать знание из данных, представляя их в виде параметризированных моделей, прямо как это делают в формулах физики\математики!
...
Реальность оказалась богаче и разнообразнее на разочарования.
...
Чаще приходится решать задачи в которых экспертные знания (читай разметка) скудны и/или на вес золота.
...
Положа руку на сердце, CVAE это нечто очень близкое к чистым классификаторам, которые на раз-два уделают любой автоэнкодер, потому что им не надо заморачиваться с какой-то там реконструкций.

Какие конкурентные преимущества дает CVAE?

Если посмотреть на [некоторые реализации CVAE](https://github.com/nnormandin/Conditional_VAE/blob/master/Conditional_VAE.ipynb), то, кажется, использование CVAE только усиливает углеродный след. Вопросы интерпретации латентных признаков не решены, а точности классификатора и реконструктора хуже.

Размышляя над тем, как и зачем можно улучшить CVAE, и перепробовав кучу подходов в духе dbscan/[нечеткой кластеризацией](https://ieeexplore.ieee.org/document/9177631)/градиентного бустинга над скрытым слоем, как то сама собой родилось понимание, что необходимо обучать VAE и выполнять кластеризацию в латентном пространстве одновременно в процессе обучения.

Оказалось, что этого можно добиться, если использовать для классификации штуку, которую мы обозвали _нечетким слоем_.

## Нечеткий слой

Сам термин "нечеткий" нами используется довольно вольготно, по факту на идея основана на принипах работы характеристической функции (или функции принадлежности).
...
Разновидностей функций принадлежности существует огромное количество на любой вкус и цвет, но зачем городить огород если есть прекрасная классическая [гауссиана](https://ru.wikipedia.org/wiki/%D0%93%D0%B0%D1%83%D1%81%D1%81%D0%BE%D0%B2%D0%B0_%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D1%8F), для одномерного случая имеющая вид
...
Пока что можно смело ожидать, что параметры этой матрицы с помощью волшебства обратного распространения ошибки в любимом ML-фреймворке подгонятся под какой-нибудь бугорок на целевом многообразии. И входные векторы $x$ тоже закучкуются под нашу $\mu(x,A)$, только данные да разметку подавай.

![Двумерная гауссиана](fig2.png)
_Тот самый бугорок в 2D_

С каждым таким гауссианом связан отдельный класс в разметке.
В такой интерпретации классы разметки можно называть нечеткими множествами.
Совокупность таких характеристических функций образует _нечеткий слой_, в коде обозначенный как `FuzzyLayer`.

Как вы могли уже догадаться, `FuzzyLayer` в процессе обучения будет одновременно стараться сгруппировать входные вектора под соответствующими бугорками функций принадлежности и адаптировать форму бугорка.

Именно это мы и собираемся проделать в скрытом пространстве вариационного энкодера.

## Вживляем нечеткость в VAE

На первый взгляд использование нечеткого слоя не сильно вмешивается в работу самого VAE, ведь и в самом деле, какая разница, где ему размещать свои распределения в латентном пространстве.
Лишь бы только размерности хватило разметить кластеры неконфликтующим образом.

Другими словами, роль `FuzzyLayer` сводится к указанию где VAE следует размещать вектора средних для распределений признаков так, чтобы они максимально соответствовали связанным нечетким множествам.
...
Вектор маски это вектор той же размерности, что и целевой вектор, но на каждой позиции находится 0, если позиция не участвуюет в вычислении невязки и 1 - если участвует.
Маска, состоящая из одних нулей означает, что данный образец при обучении проходит только через VAE, но при этом не вносит никакого вклада в структуру нечеткого слоя.
...
Маска позволяет реализовать ряд нестандарных вещей, обретающие смысл в сеттинге, когда VAE-компонента нашей модели строит компактные латентные представления входных векторов, а нечеткий слой над этим дополнительно выполняет кластеризацию в соответствии с предоставленной ему разметкой.
...
С помощью маски мы обозначили, что разметка относится только к классу `Цифра 0`, а в целевом векторе указали, что представленный пример _точно_ не является цифрой 0. Чем же является этот пример, мы не знаем, пусть нейронная сеть сама разбирается.

Такие манипуляции с масками позволяют тюнить индивидуальные гауссианы нечеткого слоя, не задевая остальные.
При этом VAE-компонента продолжает обучаться, подаем ли мы пример с разметкой или вовсе без нее.
...
Первые два слагаемые знакомы тем, кто имел уже дело с VAE, а третий компонент это сумма квадратов невязки целевого вектора с выходом нечеткого слоя, пропущенная через маску.

В большинстве случаев достаточно выбрать параметры `beta = 1` и `gamma = 1`.
...
Если же размеченных данных совсем мало, есть возможность делать два отдельных прохода обратным распространением ошибки за итерацию - сначала лоссом от чистого VAE, а затем от того, что есть в `loss_fuzzy`.
Иногда помогает.

### Момент третий - модель

Структура сети для нашего демонстрационного проекта выбрана фактически наобум - энкодер и дектор на свертках с парочкой полносвязных слоев.
Внимательный и опытный взгляд моментально обнаружит, что в энкодере и декодере нет вообще никаких отличий от VAE, поэтому их код приводить тут не будем.
Основные отличия есть в `CVAE`, которые на самом деле сводятся к применению `FuzzyLayer` в качестве классификатора.
...
class CVAE(nn.Module):
    ...
    def half_pass(self, x):
        """
        Возвращает выводы только энкодера и классификатора, без операции декодирования
        """
    ...
    def decoder_pass(self, x):
        """
        Реконструирует вывод из входного вектора х
        """
...
Другой особенностью модели является добавление к компоненту дисперсии в энкодере некоей постоянной небольшой величины `eps`. Этот прием также повышает скорость обучения сетки и ее стабильность в ряде случаев.

И последний аспект модели, на который стоит обратить внимание - нормализация батчей.
Как показывает практика, без батчнорма у автоэнкодеров есть тенденция не разворачивать латентное пространство в кластерную структуру и при особо неудачных обстоятельствах можно получить кишкообразную гиперструктуру, на которой VAE радостно скажет вам "я сделаль" и на этом успокоится.
...
В самой процедуре обучения представленной модели никаких особых хитростей нет, разве что применяется градиентный клиппинг для того, чтобы немного замедлить скорость сходимости весов сетки в надежде на более качественные итоговые результаты.

И последнее, что стоит сказать про модель это то, что она не является в классическом смысле системой нечеткого вывода, поскольку одних характеристических функций недостаточно, чтобы ее так обозвать.
Если присовокупить к `FuzzyLayer` линеный слой на входе и софтмакс на выходе, то в таком виде эту конструкцию можно выдать за нечеткий вывод Мамдани.
В этом случае латентный вектор от VAE будет представлять собой набор признаков, линейная комбинация которых активирует то или иное правило нечеткого вывода.
...
А в латентном пространстве образовались такого вида структуры, которые мы раскрасили по фактической метке класса
...
В общем, ничего интересного, но если есть желание, то можно на этом остановиться и выполнить поиск кластеров хотя бы k-means и жить дальше в проклятом мире, который сами же и создали.

Теперь давайте обучим нашу модель на полностью размеченном датасете 10 классов цифр, а 2 класса очертаний пока отключим через маску.
