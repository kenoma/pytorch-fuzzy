...
Чтобы продемонстрировать работу `FuzzyLayer` давайте попрепарируем многострадальный MNIST и ради интереса попробуем на нем построить классификатор два в одном: классификатор для цифр и не отходя от кассы классификатор, который сообщает нам, есть в начертании цифры замкнутый кружочек.
...
Полный код блокнота доступен на [гитхабе](https://github.com/kenoma/pytorch-fuzzy/blob/main/experiments_habr_part_a.ipynb), однако хочется обратить внимание на ряд моментов.
...
Маска позволяет реализовать ряд нестандарных вещей, которые имеют смысл в сеттинге, когда VAE выполняет свою кластеризацию, а нечеткий слоя это и еще группирует это под скармливаемое ему экспертное знание.
